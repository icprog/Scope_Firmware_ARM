...
static uint32_t battery_level_char_add(ble_bas_t * p_bas, const ble_bas_init_t * p_bas_init)
{
    ...
    char_uuid.uuid = BATT_LEVEL_CHAR_UUID;
    char_uuid.type = p_bas->uuid_type;
 ...
 * @return      NRF_SUCCESS on success, otherwise an error code.
 */
static uint32_t new_batteries_char_add(ble_bas_t * p_bas, const ble_bas_init_t * p_bas_init)
{
    uint32_t            err_code;
    uint8_t             initial_battery_level;
    uint8_t             encoded_report_ref[BLE_SRV_ENCODED_REPORT_REF_LEN];
    ...
    /***** Declare char UUIDs and add them to the BLE stack  *****/
    ble_uuid_t char_uuid;
    char_uuid.uuid = NEW_BATTS_CHAR_UUID;
    char_uuid.type = p_bas->uuid_type;
...
//    char_md.char_props.notify   = 1;
    ...
    err_code = sd_ble_gatts_characteristic_add(p_bas->service_handle, &char_md, &attr_char_value, &p_bas->new_batteries_handles);
...
uint32_t ble_bas_init(ble_bas_t * p_bas, const ble_bas_init_t * p_bas_init)
{
    ...
    ble_uuid128_t base_uuid = BATTERY_BASE_UUID;
    err_code = sd_ble_uuid_vs_add(&base_uuid, &(p_bas->uuid_type));
    APP_ERROR_CHECK(err_code);
    
    ble_uuid_t service_uuid;
    service_uuid.uuid = SCOPE_UUID_BATTERY;
    service_uuid.type = p_bas->uuid_type;
    ...
    err_code = sd_ble_gatts_service_add(BLE_GATTS_SRVC_TYPE_PRIMARY, &service_uuid, &p_bas->service_handle);
    if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
    ...
    // Add battery level characteristic
    new_batteries_char_add(p_bas, p_bas_init);
