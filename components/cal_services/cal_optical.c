...
static void on_write(cal_optical_t * p_optical, ble_evt_t * p_ble_evt)
{
        ...
        (p_evt_write->len <= 3))// &&(p_optical->optical_write_handler != NULL))
    {
        ...
        //p_optical->optical_write_handler(p_optical, p_evt_write->data[0]); //null pointer crashes processor...
            ...
            appData.state = APP_STATE_START_OPTICAL_CAL;
    ...
    if (p_optical->is_notification_supported)
    {
           ...
           )
        {
            ...
            if (p_optical->evt_handler != NULL)
            {
				...
				if (
            (p_evt_write->handle == p_optical->cal_optical_const_handles.cccd_handle)
            &&
            (p_evt_write->len == 2)
           )
        {
...
void squal_cal_char_add(cal_optical_t * p_optical)
{
    ...
    char_uuid.uuid      = BLE_UUID_OPTICAL_CAL_SQUAL_CHAR;
    BLE_UUID_BLE_ASSIGN(char_uuid, BLE_UUID_OPTICAL_CAL_SQUAL_CHAR);
...
//    APP_ERROR_CHECK(err_code);
    ...
    err_code = sd_ble_gatts_characteristic_add(p_optical->service_handle, &char_md, &attr_char_value, &p_optical->cal_squal_handles);
    APP_ERROR_CHECK(err_code);
}
 ...
 * @return      NRF_SUCCESS on success, otherwise an error code.
 */
static uint32_t cal_optical_const_char_add(cal_optical_t * p_optical, const cal_optical_init_t * p_optical_init)
{
    uint32_t            err_code;
    ble_gatts_char_md_t char_md;
    ble_gatts_attr_md_t cccd_md;
    ...
    err_code = sd_ble_gatts_characteristic_add(p_optical->service_handle, &char_md,
                                               &attr_char_value,
                                               &p_optical->cal_optical_const_handles);
    if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
...
uint32_t cal_optical_init(cal_optical_t * p_optical, const cal_optical_init_t * p_optical_init)
{
    ...
    err_code = cal_optical_const_char_add(p_optical, p_optical_init);
    if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
...
{
    SEGGER_RTT_printf(0, "optical_cal_const_update\n");
    if (p_optical == NULL)
    {
        return NRF_ERROR_NULL;
    }
    ...
    //{
        ...
        gatts_value.len     = sizeof(uint16_t);// sizeof(uint16_t); //was float
        gatts_value.offset  = 0;
        gatts_value.p_value = (uint8_t *)&cal_const; 

        // Update data optical.
        err_code = sd_ble_gatts_value_set(p_optical->conn_handle,
                                          p_optical->cal_optical_const_handles.value_handle,
                                          &gatts_value);
        if (err_code == NRF_SUCCESS)
        {
            //fill in
        ...
        if ((p_optical->conn_handle != BLE_CONN_HANDLE_INVALID) && p_optical->is_notification_supported)
        {
            ...
            hvx_params.handle = p_optical->cal_optical_const_handles.value_handle;
            hvx_params.type   = BLE_GATT_HVX_NOTIFICATION;
            hvx_params.offset = gatts_value.offset;
            hvx_params.p_len  = &gatts_value.len;
            hvx_params.p_data = gatts_value.p_value;
...
uint32_t squal_data_update(cal_optical_t * p_optical, uint8_t * squal_data, uint8_t size, uint8_t * bytes_sent)
{
    static int count=0;
	if (p_optical == NULL)
    {
        //return NRF_ERROR_NULL;
		SEGGER_RTT_printf(0, "error: null squal and pic input \n");
		return NRF_ERROR_INVALID_STATE;
    }
    ...
    gatts_value.len     = size*sizeof(uint8_t);
    gatts_value.offset  = 0;
    gatts_value.p_value = squal_data;

    // Update data.
    err_code = sd_ble_gatts_value_set(p_optical->conn_handle,
                                      p_optical->cal_squal_handles.value_handle,
                                      &gatts_value);
    if (err_code == NRF_SUCCESS)
    {
        //SEGGER_RTT_printf(0, "data update success \n");
    ...
    {
        SEGGER_RTT_printf(0, "error %d in squal optical update\n", err_code);
    }
    ...
    if ((p_optical->conn_handle != BLE_CONN_HANDLE_INVALID) )//&& p_ps->is_notification_supported)
    {
        ...
        hvx_params.handle = p_optical->cal_squal_handles.value_handle;
        hvx_params.type   = BLE_GATT_HVX_NOTIFICATION;
        hvx_params.offset = gatts_value.offset;
        hvx_params.p_len  = &gatts_value.len;
        hvx_params.p_data = gatts_value.p_value;
        ...
        {
            *bytes_sent = 0;
            SEGGER_RTT_printf(0, "error %d in squal cal update\n", err_code);
        }
    }
    else
    {
